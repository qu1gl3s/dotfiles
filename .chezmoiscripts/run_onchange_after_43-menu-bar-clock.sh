...
set -euo pipefail
#
# Script: run_onchange_after_43-menu-bar-clock.sh
# Purpose: Enforce menu bar clock/date preferences deterministically.
# Prerequisites: macOS, clock baseline file present in source dir.
...
if [[ "${CHEZMOI_SKIP_MACOS_DEFAULTS:-0}" == "1" ]]; then
  echo "Skipping menu bar clock settings because CHEZMOI_SKIP_MACOS_DEFAULTS=1"
  exit 0
fi
...
BASELINE_FILE="${SOURCE_DIR}/macos/clock-baseline.sh"

if [[ ! -f "${BASELINE_FILE}" ]]; then
  echo "Missing clock baseline file: ${BASELINE_FILE}" >&2
  exit 1
fi
...
if ! declare -F macos_clock_settings_entries >/dev/null 2>&1; then
  echo "macos_clock_settings_entries function not found in ${BASELINE_FILE}" >&2
  exit 1
fi
...
done < <(macos_clock_settings_entries)
