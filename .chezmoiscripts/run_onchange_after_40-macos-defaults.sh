...
set -euo pipefail
#
# Script: run_onchange_after_40-macos-defaults.sh
...
# Failure behavior: exits non-zero on malformed settings entries or write failures.

if [[ "${CHEZMOI_SKIP_MACOS_DEFAULTS:-0}" == "1" ]]; then
  echo "Skipping macOS defaults because CHEZMOI_SKIP_MACOS_DEFAULTS=1"
  exit 0
fi
